...
```
wget https://linux.dell.com/repo/community/openmanage/iSM/351/bionic/pool/main/d/dcism-osc/dcism-osc_5.0.0_amd64.deb
dpkg -i dcism-osc_5.0.0_amd64.deb 
wget https://linux.dell.com/repo/community/openmanage/iSM/351/bionic/pool/main/d/dcism/dcism_3.5.1_amd64.deb
dpkg -i dcism_3.5.1_amd64.deb 
...
snmp
apt install snmp
snmpwalk -v3 -l noAuthNoPriv -u pconwell 192.168.1.19 iso.3.6.1.2.1.33.1.2

wget https://raw.githubusercontent.com/foundObjects/pve-nag-buster/master/install.sh
bash install.sh 

smartctl -a /dev/sda
nano /etc/vzdump.conf -> prune-backups: keep-last=3

nano /etc/rsyslog.d/60-graylog.conf -> *.* @192.168.10.101:1514;RSYSLOG_SyslogProtocol23Format

apt install apcupsd
nano /etc/apcupsd/apcupsd.conf -> see below
nano /etc/default/apcupsd -> ISCONFIGURED=yes
/etc/init.d/apcupsd start
apcaccess status

pvesm prune-backups unraid --keep-last=3
pvesm prune-backups loki --keep-last=3
pvesm prune-backups local --keep-last=3
pvesm prune-backups local-lvm --keep-last=3

pvenode acme account register account-name email@email.com
